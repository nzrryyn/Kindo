...
export default function HomePage() {
  ...
  // Nama siswa yang bisa diedit per ID
  ...
  const openPenilaian = (siswa: { id: string; name: string }) => {
    ...
    // Key format: `siswaId__tahun` — load data tahun yang sedang aktif di form
    const activeTahun = currentForm.tahun || '2024/2025';
    const existing =
      allAssessments[`${siswa.id}__${activeTahun}`] ||
      allAssessments[`${siswa.id}__2024/2025`] ||
      { bb: '', tb: '', catatan: '', tahun: activeTahun, fase: 'Fondasi', aspects: {} };
  ...
  // List Siswa — ID dan nama default saja, nama display diambil fresh via getSiswaName saat render
  const allStudents = Array.from({ length: CLASS_DATA[selectedClass] }).map((_, i) => ({
    id: `${selectedClass.replace(' ', '')}_${i + 1}`,
    defaultName: `Siswa ${i + 1}`,
  }));
  // Filter pakai nama yang sudah diedit supaya search ikut nama baru
  const dummyStudents = allStudents.filter(s => {
    const displayName = getSiswaName(s.id, s.defaultName);
        ...
        {activeStudent && (
          <div className={styles.penilaianCard}>
            <div className={styles.penilaianHeader}>
              {/* Foto — klik untuk ganti, tombol X untuk hapus */}
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <div
                  className={styles.penilaianImgBox}
                  onClick={() => document.getElementById(`penilaian-photo-${activeStudent.id}`)?.click()}
                  title="Klik untuk ganti foto"
                  style={{ cursor: 'pointer' }}
                >
                  {studentPhotos[activeStudent.id]
                    ? <img src={studentPhotos[activeStudent.id]} alt={activeStudent.name} style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 15 }} />
                    : <Image src="/icongmbr.png" alt="Rapor" width={57.86} height={59.37} />
                  }
                  <input
                    id={`penilaian-photo-${activeStudent.id}`}
                    type="file" accept="image/*"
                    style={{ display: 'none' }}
                    onChange={e => handleStudentPhotoUpload(activeStudent.id, e)}
                  />
                </div>
                {/* Tombol hapus foto — muncul hanya kalau ada foto */}
                {studentPhotos[activeStudent.id] && (
                  <button
                    onClick={e => { e.stopPropagation(); handleDeleteStudentPhoto(activeStudent.id); }}
                    title="Hapus foto"
                    style={{
                      position: 'absolute', top: -6, right: -6,
                      width: 22, height: 22, borderRadius: '50%',
                      background: '#FF4343', border: '2px solid #F8F7F2',
                ...
                {editingName ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      ...
                      onKeyDown={e => {
                        if (e.key === 'Enter') handleSaveName(activeStudent.id);
                        if (e.key === 'Escape') setEditingName(false);
                      }}
                      style={{
                        fontSize: '1rem', fontWeight: 600, border: 'none',
                        borderBottom: `1.5px solid ${isDark ? '#FFB843' : '#333'}`,
                        background: 'transparent', outline: 'none',
                        color: isDark ? '#F0F0F0' : '#333', width: 140,
                        padding: '2px 0',
                      }}
                    />
                    <button
                      onClick={() => handleSaveName(activeStudent.id)}
                      style={{ background: '#FFB843', border: 'none', borderRadius: 6, padding: '2px 8px', fontSize: 11, fontWeight: 700, cursor: 'pointer', color: '#1A1A1A' }}
                    >✓</button>
                  </div>
                ) : (
                  <div
                    className={styles.penilaianName}
                    onClick={() => { setTempName(getSiswaName(activeStudent.id, activeStudent.name)); setEditingName(true); }}
                    title="Klik untuk ubah nama"
                    style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
                  >
                    {getSiswaName(activeStudent.id, activeStudent.name)}
                    {/* Ikon pensil kecil */}
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" style={{ opacity: 0.4, flexShrink: 0 }}>
            ...
            <div className={styles.barDropdowns}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (activeStudent) {
                    // Load data tahun yang dipilih jika sudah pernah disimpan
                    const existing =
                      allAssessments[`${activeStudent.id}__${newTahun}`] ||
                      { bb: '', tb: '', catatan: '', tahun: newTahun, fase: 'Fondasi', aspects: {} };
                    setCurrentForm({ ...existing, tahun: newTahun });
                  } else {
                    setCurrentForm({ ...currentForm, tahun: newTahun });
                  }
                }}
              >

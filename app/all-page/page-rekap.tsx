...
export default function LaporanPage() {
  ...
  useEffect(() => {
    ...
    fetchDokumentasi().then(setDokumentasi);
    fetchStudentNames().then(setStudentNames);

    // Realtime sinkron nama siswa
  ...
  const handleSaveName = (siswaId: string) => {
    ...
    upsertStudentName(siswaId, trimmed); // simpan ke Supabase
        ...
        <div className={styles.mainCard}>
          ...
          <div className={styles.siswaGrid}>
            {visibleSiswa.length === 0 && (
              <div className={styles.searchEmpty}>Tidak ada siswa dengan nama "{searchQuery}"</div>
            )}
              ...
              return (
                <div key={siswa.id} className={styles.siswaCard}>
                      ...
                      {editingStudentId === siswa.id ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                          <input
                            ...
                            style={{ fontSize: '13px', fontWeight: 600, border: 'none', borderBottom: '1.5px solid #FFB843', background: 'transparent', outline: 'none', color: '#F8F7F2', width: 100, padding: '2px 0', fontFamily: 'inherit' }}
                          />
                          <button onClick={() => handleSaveName(siswa.id)} style={{ background: '#FFB843', border: 'none', borderRadius: 6, padding: '2px 7px', fontSize: 11, fontWeight: 700, cursor: 'pointer', color: '#1A1A1A' }}>✓</button>
                        </div>
                      ) : (
                        <div className={styles.highlightName} onClick={() => { setTempName(displayName); setEditingStudentId(siswa.id); }} title="Klik untuk ubah nama" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                          {displayName}
                      ...
                      <div className={styles.gradeRow}>
                        {(['BSB', 'BSH', 'MB', 'BB'] as GradeKey[]).map(g => (
                          <div key={g} className={styles.gradeCell}>
                            <div className={styles.gradeSquare} style={{ backgroundColor: GRADE_COLORS[g] }}>{getGradeCount(siswa.id, g)}</div>
                  ...
                  <div className={styles.cardActions}>
                    <button className={styles.btnUnduhShortcut} onClick={() => handleUnduhPDF(siswa)} title="Unduh PDF">
                      <Image src="/unduh.svg" alt="Unduh" width={16} height={16} />
                    </button>
                    <button className={styles.btnLihat} onClick={() => { setDetailSiswa(siswa); setDetailAspek(ASPEK_LIST[0]); }}>
                      Lihat selengkapnya
                    </button>
                  </div>
                </div>
          ...
          {filteredSiswa.length > 4 && (
            <button className={styles.btnLihatSemua} onClick={() => setShowAll(prev => !prev)}>
        ...
        {detailSiswa && (
          <div className={styles.detailCard}>
                ...
                {editingStudentId === detailSiswa.id ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <input
                      ...
                      style={{ fontSize: '15px', fontWeight: 700, border: 'none', borderBottom: '1.5px solid #FFB843', background: 'transparent', outline: 'none', color: isDark ? '#F0F0F0' : '#333', width: 130, padding: '2px 0', fontFamily: 'inherit' }}
                    />
                    <button onClick={() => handleSaveName(detailSiswa.id)} style={{ background: '#FFB843', border: 'none', borderRadius: 6, padding: '2px 8px', fontSize: 11, fontWeight: 700, cursor: 'pointer', color: '#1A1A1A' }}>✓</button>
                  </div>
                ) : (
                  <div className={styles.detailName} onClick={() => { setTempName(getSiswaName(detailSiswa.id, detailSiswa.name)); setEditingStudentId(detailSiswa.id); }} title="Klik untuk ubah nama" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <div className={styles.detailDropdownRow}>
              <select className={styles.dropdownAspek} value={detailAspek} onChange={e => setDetailAspek(e.target.value)}>
                {ASPEK_LIST.map(a => <option key={a} value={a}>{a}</option>)}
              </select>
              <select className={styles.dropdownTahun} value={detailTahun} onChange={e => setDetailTahun(e.target.value)}>
                {TAHUN_LIST.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>

            {detailAspekData && (
              <div className={styles.nilaiBox} style={{ backgroundColor: GRADE_COLORS[detailAspekData.grade] }}>
                {detailAspekData.grade}
              </div>
            )}

            <div className={styles.catatanBox}>
              <div className={styles.catatanText}>{detailAssessment?.catatan || ''}</div>
            </div>
            ...
            <div className={styles.detailActions}>
              ...
              <button className={styles.btnSelesai} onClick={() => setDetailSiswa(null)}>Selesai</button>
            </div>
          </div>
        )}
      </div>
